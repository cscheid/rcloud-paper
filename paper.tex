...
Kandel's interview study points out the typical ``explore'',
``model'', ``report'' cycle~\cite{Kandel:2012:EDA}. RCloud seeks to
reduce the impedance mismatch. Kandel et al point out that larger teams
are becoming more common in data analysis, that supporting
collaboration is a difficult and important problem, and that
sharing and versioning of data sources and artifacts was hindered
by technology.  ``We found that analysts typically did not share scripts
with each other. Scripts that were shared were disseminated similarly to
intermediate data: either through shared drives or email.
Analysts rarely stored their analytic code in source control.''
Their work points to the need for better technology to support
these aspects of data analysis in teams.

Heer and Agarwala identify a large number of design considerations for
...
Section~\ref{sec:notebooks}, addresses modularity and granularity, and
artifact histories.

Our work has been inspired or has benefited from other modern technologies
to improve the data analysis environments and processes, such as RStudio,
R packages such as Markdown, knitr and Shiny, and iPython notebooks, to name a few.  % explain
...
% limitations
Previous studies have pointed out frequent difficulties in the
achieving production-level flexibility, scalability and maintainability
with experimental code created by data analysts.  These properties cannot
be ensured by any programming environment, but a reasonable approach in
general is to provide suitable tools that make it easier for well-intentioned
programmers to adopt such practices.
