...
The starting point in our design was to take advantage of the current
infrastructure of web software as much as possible. HTTP, despite its
deficiencies, has become the lingua franca of
distributed interprocess communication. This meant that if our system
could speak HTTP natively, then it would be possible to provide a low-friction
path from development of an interactive data analysis script to an
automated \emph{web service}, essentially a remote function call over
...
Section~\ref{sec:related}, this is not novel; executable
documents like this are a feature of many other systems,
...
an experience similar to running a shell script.
...
the notebook is being developed. Another way to describe this is that
...
When we combine immediate deployment with the ability to refer to the
latest version of a notebook by name, we get notebooks that are always live,
...
that film, then you should like this one too''). To compute such
...
always available, to encourage its use.
%
