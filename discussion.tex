...
Some aspects of the programming environment are more difficult to
manage in RCloud than in conventional systems. RCloud does not separate
the development and deployment environments, and every version of every
prototype is shared by default. While this encourages
collaboration, it also quickly exposes errors such as
misconfiguration, mismatches between package versions, and
programming errors, that can unintentionally affect production
websites. For example, if an RCloud website is distributed across
multiple servers, conflicting versions of system software
can cause errors that are hard to debug.
...
strong.) At the other end of the scale, there is hardly any version
control for the external environment, and if an external service 
...
open research topics~\cite{Moore:2011:SAF}.
...
is getting saved}, the extra versions become burdensome to them.
