...
\paragraph*{Discovery is hard} There are several problems.
With hundreds of users and thousands of notebooks, the notebook
tree becomes unwieldy.
The search function does not protect searchers from obsolete and
erroneous notebooks, which by default are retained forever.
It helps with finding notebooks, but not with acquiring
other types of information, such as methodology.

One thing to try is tagging, so that users can mark notebooks
to make them easier to find. However, this relies on users making
a conscious effort, which is not always reliable. It seems better
to rely more on passive metrics, such as how often a notebook has been run.
...
should be informed. Operations on multiple notebooks and folders of
...
poses a problem, too, when dozens of versions of a notebook proliferate.
A possible solution is to allow overwriting the current state
of a notebook with the current state of another fork of the notebook.

Implementing a more general solution seems like a big job.
Even GitHub's web interface does not support merging where there are
conflicts. Unrestricted sharing of notebooks may involve moving
toward git's notion of decentralized code repositories, because GitHub
gists do not support shared ownership.
